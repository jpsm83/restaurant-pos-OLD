...
export const GET = async (req: Request) => {
  try {
    ...
    const printers = await Printer.aggregate([
      // Step 1: Unwind configurationSetupToPrintOrders array
      { $unwind: "$configurationSetupToPrintOrders" },
      // Step 2: Unwind businessSalesLocationReferenceIds array
      {
        $unwind:
      ...
      // Step 4: Lookup to fetch Users based on excludeUserIds
      {
        $lookup: {
