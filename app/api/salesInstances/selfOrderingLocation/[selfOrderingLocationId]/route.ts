...
// @access  Private
export const POST = async (req: Request, context: { params: { selfOrderingLocationId: Types.ObjectId } }) => {
    // 1. user will scan the QR code
    // 2. if your has an accout it will be redirected to the selfOrdering page
    // 3. if not, the user will be redirected to the register page (with google or facebook)
    // 4. he will be redirect to the selfOrdering page
    // 5. the user will select what he wants to eat and drink
    // 6. the user will pay for the order
    // 7. the user will receive a confirmation message with the order number
    // 8. the order will be done and delivered to the user in the salesPoint location
    try {
      ...
      // validate ids
      if (isObjectIdValid([selfOrderingLocationId, openedById, businessId]) !== true) {
        return new NextResponse(
          JSON.stringify({
      ...
      const newSalesInstanceObj = {
        ...
        status: "Occupied",
        businessId,
        clientName: "get the client name on the user query",
      };
